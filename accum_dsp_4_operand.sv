...
* :RST_EN: if set to 1, the output reset is available via rst_n (allowing for 
* resetting an accumulating circuit without deasserting i_accumulate). Has no 
* effect if not (OUT_REG==1 || ACCUMULATE_EN==1), because in those cases the 
* register to be reset is deactivated
...
import nfe_engine_pkg::*;

module accum_dsp_4_operand #(
    parameter                                       DATA_WIDTH = DSP_MULT_FULL_WIDTH,
    parameter                                       RST_EN = 0,
    ...
    logic                                           rst_n,
    logic                                           i_accumulate,
    logic [DATA_WIDTH-1:0]                          i_operands  [4],
    ...
    logic [DSP_MULT_FULL_WIDTH-1:0]         operands_full_width [4];

    logic [DSP_MULT_FULL_WIDTH-1:0]         dsp_cascade;
    logic                                   dsp_mult_sign;
    logic [DSP_MULT_IN_WIDTH_WIDE-1:0]      dsp_op_a    [2];
    logic [DSP_MULT_IN_WIDTH_NARROW-1:0]    dsp_op_b    [2];
    ...
    // * everything else 0
    logic [1:0] bits_opmode_w_dsp_master = {1'b0, dsp_master_accumulate_en};
    ...
    logic [DSP_MULT_FULL_WIDTH-1:0]         dummy_dsp_slave_result;
    logic [DSP_MULT_FULL_WIDTH-1:0]         dummy_dsp_cascade;
    ...
    logic [DSP_MULT_FULL_WIDTH-1:0]         dummy_dsp_master_cascadeout;
    ...
    logic [DSP_MULT_FULL_WIDTH-1:0]         dummy_dsp_slave_cascadeout;
    ...
    generate begin: gen_operand_padding
        for (i=0; i<4; i++) begin
            assign operands_full_width[i] =
                {{(DSP_MULT_FULL_WIDTH-DATA_WIDTH){i_operands[i][DATA_WIDTH-1]}}, i_operands[i]};
        end
    end endgenerate
    ...
    assign dsp_op_a[0] = i_operand[0][DSP_MULT_FULL_WIDTH-1 -: 30];
    assign dsp_op_b[0] = i_operand[0][17:0];
    assign dsp_op_a[1] = i_operand[2][DSP_MULT_FULL_WIDTH-1 -: 30];
    assign dsp_op_b[1] = i_operand[2][17:0];

    // TODO: operand to DSP signal assignment
    ...
    DSP48E2 #(
        ...
        .D({DSP_D_WIDTH{1'b1}}), // dummy
        // DATA OUTPUTS
        .CARRYOUT(dummy_dsp_master_carryout),
        .P(o_result),
        .XOROUT(dummy_dsp_master_xorout),
        // CASCADE INPUTS
        .ACIN({DSP_A_WIDTH{1'b1}}), // dummy
        .BCIN({DSP_B_WIDTH{1'b1}}), // dummy
        .CARRYCASCIN('1), // dummy
